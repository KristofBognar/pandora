function merge_pandora_dscds( p_num, year, uvvis, version )
%reads in and merges Pandora dSCDs for given instrument and year
...
% save folder -- filedir changes for v5
savedir=filedir;

% make list of all files
if version<5 % multi-munth files from QDOAS GUI
    tmp = dir([filedir 'Pandora_' num2str(p_num) '_' uvvis '_' num2str(year) '_*.ASC']); 
else % daily files from QDOAS command line tool
    filedir=[filedir 'p' num2str(p_num) '_' meas_type '/'];
    tmp = dir([filedir 'OFFAXIS_' meas_type '_*_' num2str(year) '*.ASC']); 
end
...
for i=1:length(f_list)
    ...
    disp_str=['File ', num2str(i), '/', num2str(size(f_list,2)), ': ', f_list{i}];
    ...
    if version<5
        switch uvvis
            case 'vis'
                tmp=read_pandora_vis_v2(filedir,f_list{i});
            case 'uv'
...
save([savedir 'Pandora_' num2str(p_num) '_' uvvis '_' num2str(year) '.mat'],'data');
    

end


function data = read_pandora_vis_v2(path, filename, do_save)
%read_pandora_v2(path, filename)
...
function data = read_pandora_vis_v5(path, filename)
...
formatSpec = '%f%f%f%f%f%f%f%f%{dd/MM/yyyy}D%{HH:mm:ss}D%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%[^\n\r]';
...
%% Create output variable
data = table(dataArray{1:end-1}, 'VariableNames', {'SpecNo','Year','Fractionalday','Fractionaltime','SZA','SolarAzimuthAngle','Elevviewingangle','Azimviewingangle','DateDDMMYYYY','Timehhmmss','Tint',...
       'TotalExperimentTimesec','NO2_VisRMS','NO2_VisRefZm','NO2_VisSlColo3','NO2_VisSlErro3','NO2_VisSlColno2','NO2_VisSlErrno2','NO2_VisSlColno2a','NO2_VisSlErrno2a','NO2_VisSlColo4','NO2_VisSlErro4',...
       'NO2_VisSlColh2o','NO2_VisSlErrh2o','NO2_VisSlColring','NO2_VisSlErrring','NO2_VisShiftSpectrum','NO2_VisStretchSpectrum1','NO2_VisStretchSpectrum2','NO2_UVRMS','NO2_UVRefZm','NO2_UVSlColno2',...
       'NO2_UVSlErrno2','NO2_UVSlColno2a','NO2_UVSlErrno2a','NO2_UVSlColo3','NO2_UVSlErro3','NO2_UVSlColo3a','NO2_UVSlErro3a','NO2_UVSlColo4','NO2_UVSlErro4','NO2_UVSlColhcho','NO2_UVSlErrhcho',...
